...
def pbc_to_cgc(arr):
    ...
    """
    log.critical('Chuyen sang anh grayscale...')
...
def cgc_to_pbc(arr):
    ...
    assert cgc_to_pbc(pbc_to_cgc(arr)) == arr
    """
    log.critical('Chuyen anh xam cho lai anh mau...')
...
class BitPlane:
    ...
    def slice(self, nbits):
        ...
        """
        log.critical('Dang chia anh thanh cac bit-plane...')
    ...
    def stack(self):
        ...
        def iterate_all_but_last_dim(arr):
            ...
            for ind in itertools.product(*all_inds[:-1]):
                yield ind
        log.critical('Dang gop cac bit-plane lai thanh pixel anh that....')
