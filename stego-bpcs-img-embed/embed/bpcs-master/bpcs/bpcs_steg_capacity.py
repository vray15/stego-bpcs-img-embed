...
class HistogramComplexityImage(ActOnImage):
    def modify(self, alpha, comp_fcn, grid_size=(8,8)):
        ...
        log.critical('{0} grids trong {1} grids vung đat đo phuc tap du cao > {2}'.format(navail, ntotal, alpha))
        ...
        log.critical('Hinh anh nay có the chua khoang {0} bytes.'.format(nbytes))
        log.critical('{0} byte tin nhan se su dung {1:.1%} dung luong anh co the dau.'.format(nbytes, percent))
        return hist
